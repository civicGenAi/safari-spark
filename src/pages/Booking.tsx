...
const BookingPage = () => {
  ...
  const inputClass = (field: string) =>
    `w-full bg-surface-2 border ${errors[field] ? "border-destructive animate-[shake_0.3s_ease-in-out]" : "border-border"} rounded-full px-5 py-3 text-foreground text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all font-body`;
        ...
        <div className="container-max max-w-2xl">
          <AnimatedSection>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-sm text-muted-foreground mb-2 font-body">Full Name *</label>
                <input name="name" value={form.name} onChange={handleChange} className={inputClass("name")} placeholder="John Doe" />
              </div>

              <div>
                <label className="block text-sm text-muted-foreground mb-2 font-body">Phone Number *</label>
                <input name="phone" value={form.phone} onChange={handleChange} className={inputClass("phone")} placeholder="+255 ..." type="tel" />
              </div>

              <div>
                <label className="block text-sm text-muted-foreground mb-2 font-body">Service Type *</label>
              ...
              <div>
                <label className="block text-sm text-muted-foreground mb-2 font-body">Pickup Location *</label>
                <input name="pickup" value={form.pickup} onChange={handleChange} className={inputClass("pickup")} placeholder="e.g. JRO Airport" />
              </div>

              <div>
                <label className="block text-sm text-muted-foreground mb-2 font-body">Drop-off / Destination *</label>
                <input name="dropoff" value={form.dropoff} onChange={handleChange} className={inputClass("dropoff")} placeholder="e.g. Arusha" />
              </div>

              <div>
                <label className="block text-sm text-muted-foreground mb-2 font-body">Date & Time *</label>
                <input name="date" value={form.date} onChange={handleChange} className={inputClass("date")} type="datetime-local" />
              </div>

              <div>
                <label className="block text-sm text-muted-foreground mb-2 font-body">Number of Passengers</label>
              ...
              <div>
                <label className="block text-sm text-muted-foreground mb-2 font-body">Special Requests</label>
              ...
              <Button variant="gold" size="xl" type="submit" className="w-full" disabled={loading}>
                {loading ? (
                  <><Loader2 size={20} className="animate-spin" /> Sending...</>
                ) : (
                  <><Send size={20} /> Send Booking via WhatsApp</>
                )}
              </Button>
            </form>
          </AnimatedSection>
